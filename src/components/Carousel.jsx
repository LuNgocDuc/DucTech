        ...
        <Slider {...settings}>
            {
                data?.slice(0,7)?.map((item, index) => {
                    return <div key={index} className='bg-gradient-to-r from-[#0f0c29] via-[#b04877] to-[#411232] -z-10'>
                        <div className='flex flex-col md:flex-row gap-10 justify-center h-[600px] my-20 md:my-0 items-center px-4'>
                            <div className='md:space-y-6 space-y-3'>
                                <h3 className='text-red-500 font-semibold font-sans text-sm'>Level Up Technology with the Best Electronics</h3>
                                ...
                                <button onClick={() => navigate(`/products/${item._id}`)} className='bg-gradient-to-r from-[#0f056f] via-[#1b0a40] to-[#1a0b72] text-white px-3 py-2 rounded-md cursor-pointer mt-2'>Mua ngay</button>
                            </div>
                            <div>
                                <img src={item.image} alt={item.title} className='w-[350px] h-auto object-contain rounded-xl shadow-lg hover:scale-105 transition-all duration-300 shadow-2xl shadow-indigo-950' />
                            </div>
                        </div>
                    </div>
                })
