...
const Cart = ({ location, getLocation }) => {
  ...
  const totalPrice = cartItem.reduce((total, item) => {
    // Lấy giá trị số (price) và số lượng (quantity), mặc định là 0 hoặc 1 nếu không tồn tại
    const price = Number(item.discountedPrice) || 0;
    const quantity = Number(item.quantity) || 1;

    // Tính tổng: total + (giá * số lượng)
        ...
        <div>
          ...
          <div className="grid grid=cols-1 md:grid-cols-2 md:gap-20">
            <div className="bg-gray-100 rounded-md p-7 mt-4 space-y-2">
              ...
              <div className="flex items-center justify-center w-full text-gray-700"></div>
